...
import RosterMatchHistory from "./RosterMatchHistory";
...
import { calculateRosterMatches } from "../utils/rosterMatchHistory";
...
export default function RosterView({ playerData }) {
  ...
  const [activeMode, setActiveMode] = useState("manage"); // "manage" | "compare"
  ...
  const [activeSubTab, setActiveSubTab] = useState("synergy"); // "synergy" | "agents" | "maps" | "history"
  ...
  // Apply date range filter globally to analysis matches
  const filteredAnalysisMatches = filtrarPorRango(analysisMatches, dateRange);

  // Hook for synergy matrix calculations using filtered matches
  const { synergyPairs, rosterSynergy, rosterAgentStats } = useSynergyMatrix(
    filteredAnalysisMatches,
    analysisPuuid,
    activeRoster
  );

  // Roster map round-level stats
  const mapStats = calculateRosterMapStats(filteredAnalysisMatches, activeRoster);

  // Roster team match history (requires 3+ members)
  const rosterHistory = calculateRosterMatches(filteredAnalysisMatches, activeRoster);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", width: "100%" }}>
      {/* Mode Selector Tabs */}
      <div style={{ display: "flex", borderBottom: "1px solid var(--line)", gap: "16px", paddingBottom: "10px" }}>
        <button
          ...
          style={{
            ...
            border: "none",
            borderBottom: activeMode === "manage" ? "2px solid var(--red)" : "2px solid transparent",
            padding: "8px 16px",
            color: activeMode === "manage" ? "white" : "var(--text-dim)",
            fontSize: "14px",
            cursor: "pointer",
            letterSpacing: "0.5px",
            display: "flex",
        ...
        <button
          ...
          style={{
            background: "transparent",
            border: "none",
            borderBottom: activeMode === "compare" ? "2px solid var(--red)" : "2px solid transparent",
            padding: "8px 16px",
            color: activeMode === "compare" ? "white" : "var(--text-dim)",
            fontSize: "14px",
            cursor: "pointer",
            letterSpacing: "0.5px",
            display: "flex",
      ...
      {activeMode === "compare" ? (
        <RosterComparison rosters={rosters} matches={analysisMatches} activePuuid={activePlayer?.puuid} />
      ) : (
        <div style={{
          display: "flex",
          ...
          <div style={{ flex: 1, padding: "20px", display: "flex", flexDirection: "column", gap: "20px", overflowX: "hidden" }}>
            {!activeRoster ? (
              ...
              <>
                ...
                {/* Focus player selector and Date Range Filter */}
                <div style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                ...
                <div style={{ display: "flex", borderBottom: "1px solid var(--line)", gap: "16px", marginTop: "10px" }}>
                  ...
                  <button
                    ...
                    style={{
                      background: "transparent",
                      border: "none",
                      borderBottom: activeSubTab === "maps" ? "2px solid var(--red)" : "2px solid transparent",
                      padding: "8px 0",
                      color: activeSubTab === "maps" ? "white" : "var(--text-dim)",
                      fontSize: "14px",
                      cursor: "pointer",
                      letterSpacing: "0.5px"
                    }}
                  ...
                  <button
                    ...
                    style={{
                      background: "transparent",
                      border: "none",
                      borderBottom: activeSubTab === "history" ? "2px solid var(--red)" : "2px solid transparent",
                      padding: "8px 0",
                      color: activeSubTab === "history" ? "white" : "var(--text-dim)",
                      fontSize: "14px",
                      cursor: "pointer",
                      letterSpacing: "0.5px"
                    }}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {activeSubTab === "maps" && (
                    <MapasTab mapStats={mapStats} />
                  )}

                  {activeSubTab === "history" && (
                    <RosterMatchHistory history={rosterHistory} />
                  )}
                </div>
              </>
            )}
